...
const msgEmty= document.querySelector('.msg-empty');
...
function addToCart(){
    ...
    <div class="item">
        ...
        <div class="details">
            <div class="product-name"> Bahar Fırsatı Sınırlı Üretim...</div>
...
function updateMsgEmpty(){
    if(productsInCart ==0){
        if(msgEmty.classList.contains('hidden')){
            msgEmty.classList.remove('hidden');
        }
    } else {
        if(!msgEmty.classList.contains('hidden')){
            msgEmty.classList.add('hidden');
        }
    }
